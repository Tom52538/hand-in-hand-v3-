    ...
    router.get('/create-monthly-pdf', isAdmin, async (req, res) => {
        ...
        try {
            ...
            if (allDays.length === 0) { doc.text('Keine Buchungen/Abwesenheiten.', doc.page.margins.left, doc.y); doc.y += TABLE_ROW_HEIGHT; }
                ...
                for (let i = 0; i < allDays.length; i++) {
                    ...
                    if ((doc.y + spaceNeededForRest > doc.page.height - doc.page.margins.bottom && i > 0) || (doc.y + TABLE_ROW_HEIGHT > doc.page.height - doc.page.margins.bottom - summaryHeight - footerHeight)) {
                        drawFooter(doc, doc.page.height - doc.page.margins.bottom - footerHeight + V_SPACE.MEDIUM); doc.addPage(); currentY = doc.page.margins.top;
                    ...
                    // *** KORRIGIERTER Aufruf von getExpectedHours ***
                    const expectedHours = getExpectedHours(employeeDataForPdf, dayData.date);
                    ...
                    const currentRowY = doc.y; const textOptions = { align: 'center', lineBreak: false };
                    doc.text(dateFormatted, colPositions.date, currentRowY, { ...textOptions, width: colWidths.date, align: 'left' });
                    doc.text(startOverride, colPositions.start, currentRowY, { ...textOptions, width: colWidths.start }); doc.text(endOverride, colPositions.end, currentRowY, { ...textOptions, width: colWidths.end });
                    doc.text(expectedStr, colPositions.expected, currentRowY, { ...textOptions, width: colWidths.expected }); doc.text(actualStr, colPositions.actual, currentRowY, { ...textOptions, width: colWidths.actual }); doc.text(diffStr, colPositions.diff, currentRowY, { ...textOptions, width: colWidths.diff });
            ...
            // Zusammenfassung (Monat)
             ...
             doc.y = currentY;
             doc.font(FONT_BOLD).fontSize(FONT_SIZE.SUMMARY); const summaryLabelWidth = colWidths.date + colWidths.start + colWidths.end + colWidths.expected - V_SPACE.SMALL; const summaryValueWidth = colWidths.actual + colWidths.diff; const summaryLabelX = doc.page.margins.left; const summaryValueX = colPositions.actual; const summaryLineSpacing = 0.2;
