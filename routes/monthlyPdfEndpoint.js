...
function isAdmin(req, res, next) {
  if (req.session && req.session.isAdmin) {
    ...
    console.warn("[isAdmin Middleware] Zugriff verweigert. Keine Admin-Session.");
...
module.exports = function (db) {
    ...
    // GET /create-monthly-pdf (V10 - Check tableLayout, Mit Debugging)
    router.get('/create-monthly-pdf', isAdmin, async (req, res) => {
        ...
        try {
            ...
            if (!name || !year || !month || isNaN(parseInt(year)) || isNaN(parseInt(month)) || month < 1 || month > 12) {
                console.error(`[PDF Mon V10 DEBUG] Ungültige Parameter: name=${name}, year=${year}, month=${month}`);
            ...
            // +++ DEBUG LOGGING +++
            console.log('[PDF Mon V10 DEBUG] ===> Vor calculateMonthlyData...');
            // Daten holen
            const data = await calculateMonthlyData(db, name, year, month);
            // +++ DEBUG LOGGING +++
            console.log('[PDF Mon V10 DEBUG] <=== Nach calculateMonthlyData. Daten erhalten:', data ? `Ja (${data.workEntries?.length || 0} Arbeits-Einträge, ${data.absenceEntries?.length || 0} Abwesenheiten)` : 'Nein / Fehler');
            if (!data) {
                throw new Error("Konnte Daten für die PDF-Generierung nicht abrufen.");
            }
            ...
            console.log(`[PDF Mon V10 DEBUG] Seite ${currentPage} hinzugefügt.`);
            ...
            if (!tableLayout || !tableLayout.colWidths || !tableLayout.colPositions) { throw new Error(`[PDF V10 Error] drawTableHeader returned invalid layout object on page ${currentPage} (initial).`); }
            ...
            if (allDays.length === 0) {
                ...
                console.log(`[PDF Mon V10 DEBUG] Starte Zeichnen von ${allDays.length} Zeilen...`);
                for (let i = 0; i < allDays.length; i++) {
                    // +++ DEBUG LOGGING +++
                    if (i % 10 === 0 || i === allDays.length - 1) {
                         console.log(`[PDF Mon V10 DEBUG] --- Zeichne Zeile ${i+1}/${allDays.length} für ${allDays[i].date}`);
                    }
                    // --- Ende Debug Logging ---
                    ...
                    if (doc.y + estimatedLineHeight > doc.page.height - doc.page.margins.bottom - FOOTER_TOTAL_HEIGHT - SUMMARY_TOTAL_HEIGHT) { // Mehr Platz lassen
                        doc.addPage(); currentPage++; console.log(`[PDF Mon V10 DEBUG] Seite ${currentPage} manuell hinzugefügt (vor Zeile ${i+1}).`);
                        drawPageNumber(doc, currentPage); currentY = doc.page.margins.top;
                        tableLayout = drawTableHeader(doc, currentY, usableWidth);
                        if (!tableLayout || !tableLayout.colWidths || !tableLayout.colPositions) { throw new Error(`[PDF V10 Error] drawTableHeader returned invalid layout object on page ${currentPage} (after page break).`); }
                    ...
                    const currentRowY = doc.y; const { colPositions, colWidths } = tableLayout;
                ...
                console.log(`[PDF Mon V10 DEBUG] Zeichnen der Zeilen beendet.`);
            }
            ...
            try {
                ...
                const summaryLabelWidth = tableLayout.colWidths.date + tableLayout.colWidths.start + tableLayout.colWidths.end + tableLayout.colWidths.expected - V_SPACE.SMALL; const summaryValueWidth = tableLayout.colWidths.actual + tableLayout.colWidths.diff; const summaryLabelX = doc.page.margins.left; const summaryValueX = tableLayout.colPositions.actual; const summaryLineSpacing = 0.5;
            ...
            // --- PDF abschließen ---
            // +++ DEBUG LOGGING +++
            console.log("[PDF Mon V10 DEBUG] ===> Kurz vor doc.end()");
            doc.end();
            console.log("[PDF Mon V10 DEBUG] ===> Nach doc.end() aufgerufen.");

        } catch (err) {
            // *** Fehlerbehandlung (V10 - Robust) ***
            console.error("[PDF Mon V10 DEBUG] !!!!! CATCH BLOCK REACHED (MONTHLY) !!!!!");
            // +++ DEBUG LOGGING (Mehr Details) +++
            console.error("Fehler Erstellen Monats-PDF V10:", err); // Gibt ganzes Fehlerobjekt aus
            if (!res.headersSent) {
    ...
    //-----------------------------------------------------
     ...
     // GET /create-period-pdf (V10 - Check tableLayout, Mit Debugging)
        ...
        let doc; let currentPage = 0;
         try {
            ...
            if (!name || !year || isNaN(parseInt(year)) || !periodType || !['QUARTER', 'YEAR'].includes(periodType.toUpperCase())) {
                 console.error(`[PDF Per V10 DEBUG] Ungültige Parameter: name=${name}, year=${year}, periodType=${periodType}`);
            ...
            if (periodType.toUpperCase() === 'QUARTER' && (!periodValue || isNaN(parseInt(periodValue)) || periodValue < 1 || periodValue > 4)) {
                 console.error(`[PDF Per V10 DEBUG] Ungültiger periodValue für Quartal: ${periodValue}`);
            ...
            // +++ DEBUG LOGGING +++
            console.log('[PDF Per V10 DEBUG] ===> Vor calculatePeriodData...');
            // Daten holen
            const data = await calculatePeriodData(db, name, year, pTypeUpper, pValue);
            // +++ DEBUG LOGGING +++
            console.log('[PDF Per V10 DEBUG] <=== Nach calculatePeriodData. Daten erhalten:', data ? `Ja (${data.workEntriesPeriod?.length || 0} Arbeits-Einträge, ${data.absenceEntriesPeriod?.length || 0} Abwesenheiten)` : 'Nein / Fehler');
             if (!data) {
                throw new Error("Konnte Daten für die PDF-Generierung nicht abrufen.");
            }
            const employeeDataForPdf = data.employeeData;
            ...
            // Erste Seite hinzufügen und Kopf/Seitenzahl
            currentPage++;
            doc.addPage(); console.log(`[PDF Per V10 DEBUG] Seite ${currentPage} hinzugefügt.`);
            ...
            if (allDaysPeriod.length === 0) {
                 ...
                 console.log(`[PDF Per V10 DEBUG] Starte Zeichnen von ${allDaysPeriod.length} Zeilen...`);
                 for (let i = 0; i < allDaysPeriod.length; i++) {
                     // +++ DEBUG LOGGING +++
                     if (i % 10 === 0 || i === allDaysPeriod.length - 1) {
                         console.log(`[PDF Per V10 DEBUG] --- Zeichne Zeile ${i+1}/${allDaysPeriod.length} für ${allDaysPeriod[i].date}`);
                     }
                     // --- Ende Debug Logging ---
                    ...
                    const estimatedLineHeight = TABLE_ROW_HEIGHT;
                    if (doc.y + estimatedLineHeight > doc.page.height - doc.page.margins.bottom - FOOTER_TOTAL_HEIGHT - SUMMARY_TOTAL_HEIGHT) { // Mehr Platz lassen
                        doc.addPage(); currentPage++; console.log(`[PDF Per V10 DEBUG] Seite ${currentPage} manuell hinzugefügt (vor Zeile ${i+1}).`);
                        drawPageNumber(doc, currentPage); currentY = doc.page.margins.top;
                        tableLayout = drawTableHeader(doc, currentY, usableWidth);
                        if (!tableLayout || !tableLayout.colWidths || !tableLayout.colPositions) { throw new Error(`[PDF V10 Error] drawTableHeader returned invalid layout object on page ${currentPage} (after page break).`); }
                        currentY = tableLayout.headerBottomY; doc.font(FONT_NORMAL).fontSize(FONT_SIZE.TABLE_CONTENT).lineGap(1.5); doc.y = currentY;
                ...
                console.log(`[PDF Per V10 DEBUG] Zeichnen der Zeilen beendet.`);
            }
            ...
            try {
                doc.font(FONT_BOLD).fontSize(FONT_SIZE.SUMMARY).lineGap(0);
                if (!tableLayout || !tableLayout.colWidths || !tableLayout.colPositions) { throw new Error(`[PDF V10 Error] Invalid tableLayout object before drawing summary on page ${currentPage}.`); }
                const summaryLabelWidth = tableLayout.colWidths.date + tableLayout.colWidths.start + tableLayout.colWidths.end + tableLayout.colWidths.expected - V_SPACE.SMALL; const summaryValueWidth = tableLayout.colWidths.actual + tableLayout.colWidths.diff; const summaryLabelX = doc.page.margins.left; const summaryValueX = tableLayout.colPositions.actual; const summaryLineSpacing = 0.5; const periodLabelSummary = data.periodIdentifier || pTypeUpper;
            ...
            // --- PDF abschließen ---
             // +++ DEBUG LOGGING +++
            console.log("[PDF Per V10 DEBUG] ===> Kurz vor doc.end()");
            doc.end();
            console.log("[PDF Per V10 DEBUG] ===> Nach doc.end() aufgerufen.");

        } catch (err) {
            // *** Fehlerbehandlung (V10 - Robust) ***
            console.error("[PDF Per V10 DEBUG] !!!!! CATCH BLOCK REACHED (PERIOD) !!!!!");
            // +++ DEBUG LOGGING (Mehr Details) +++
            console.error("Fehler Erstellen Perioden-PDF V10:", err); // Gibt ganzes Fehlerobjekt aus
            if (!res.headersSent) {
