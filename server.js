...
// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging) - Teil 2
...
app.use(session({
    ...
    cookie: {
        ...
        sameSite: 'lax' // Explizit setzen ('lax' ist guter Standard)
    }
}));
...
// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging) - Teil 3
...
// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging) - Teil 4
...
app.post('/log-start', async (req, res) => {
    ...
    try {
        ...
        // const checkCompleteQuery = `SELECT id FROM work_hours WHERE LOWER(name) = LOWER($1) AND date = $2 AND endtime IS NOT NULL`; // Auskommentiert für Mehrfachbuchungen
...
// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging) - Teil 5
...
app.get('/admin-work-hours', isAdmin, async (req, res) => {
    ...
    if (employeeId && employeeId !== 'all' && employeeId !== '') {
        const empIdInt = parseInt(employeeId); if (isNaN(empIdInt)) { console.error(`${logPrefix} ERROR - Invalid employeeId.`); return res.status(400).json({ message: 'Ungültige Mitarbeiter-ID.'});}
    ...
    if (year && month) {
        ...
        if (isNaN(parsedYear) || isNaN(parsedMonth) || parsedMonth < 1 || parsedMonth > 12 || String(parsedYear).length !== 4) { console.error(`${logPrefix} ERROR - Invalid year/month.`); return res.status(400).json({ message: 'Ungültiges Jahr/Monat.' }); }
        try {
        ...
        } catch(dateError) { console.error(`${logPrefix} ERROR - Date processing error:`, dateError); return res.status(400).json({ message: `Datumsfehler für ${year}-${month}.` }); }
    }
...
app.get('/admin-download-csv', isAdmin, async (req, res) => {
    ...
    if (year && month) {
        const parsedYear = parseInt(year); const parsedMonth = parseInt(month); if (isNaN(parsedYear) || isNaN(parsedMonth) || parsedMonth < 1 || parsedMonth > 12 || String(parsedYear).length !== 4) { console.error(`${logPrefix} ERROR - Invalid year/month.`); return res.status(400).json({ message: 'Ungültiges Jahr/Monat.' }); }
...
app.put('/api/admin/update-hours', isAdmin, async (req, res) => {
  ...
  const netHours = calculateWorkHours(startTime, endTime); if (netHours < 0) console.warn(`${logPrefix} Negative work hours calculated. Saving 0.`);
  try {
      console.log(`${logPrefix} Checking if entry exists...`);
      const checkResult = await db.query('SELECT 1 FROM work_hours WHERE id = $1', [entryId]); if (checkResult.rows.length === 0) { console.warn(`${logPrefix} Entry not found.`); return res.status(404).json({ message: `Eintrag ID ${entryId} nicht gefunden.` });}
      ...
      if (result.rowCount > 0) { console.log(`${logPrefix} Update successful.`); res.status(200).send('Eintrag aktualisiert.'); }
      else { console.warn(`${logPrefix} Update failed (rowCount=0).`); res.status(404).send(`Eintrag ID ${entryId} nicht aktualisiert.`); }
...
app.delete('/api/admin/delete-hours/:id', isAdmin, async (req, res) => {
  ...
  console.log(`${logPrefix} Request received.`);
  // +++ LOGGING ENDE +++
  const { id } = req.params; const entryId = parseInt(id); if (isNaN(entryId)) { console.error(`${logPrefix} ERROR - Invalid ID.`); return res.status(400).send('Ungültige ID.'); }
  try {
    console.log(`${logPrefix} Deleting entry from DB...`);
    const result = await db.query('DELETE FROM work_hours WHERE id = $1', [entryId]);
    if (result.rowCount > 0) { console.log(`${logPrefix} Delete successful.`); res.status(200).send('Eintrag gelöscht.'); }
    else { console.warn(`${logPrefix} Entry not found.`); res.status(404).send(`Eintrag ID ${entryId} nicht gefunden.`); }
...
app.delete('/adminDeleteData', isAdmin, async (req, res) => {
  ...
  // +++ LOGGING ENDE +++
  // Sicherheitsabfrage im Frontend empfohlen, hier nur rudimentär
  // const confirmation = req.body.confirmation; // Sicherer: Bestätigung im Body erwarten
  // if (confirmation !== 'LÖSCHEN') {
  //    console.warn(`${logPrefix} Confirmation missing or incorrect. Aborting.`);
  //    return res.status(403).send("Bestätigung 'LÖSCHEN' im Request Body fehlt oder ist falsch. Abbruch.");
  // }
  let client;
  try {
      client = await db.connect(); await client.query('BEGIN');
      console.warn(`${logPrefix} Deleting monthly_balance...`); const resultMB = await client.query('DELETE FROM monthly_balance'); console.warn(`${logPrefix} -> ${resultMB.rowCount} rows deleted.`);
      console.warn(`${logPrefix} Deleting absences...`); const resultAbs = await client.query('DELETE FROM absences'); console.warn(`${logPrefix} -> ${resultAbs.rowCount} rows deleted.`);
      console.warn(`${logPrefix} Deleting work_hours...`); const resultWH = await client.query('DELETE FROM work_hours'); console.warn(`${logPrefix} -> ${resultWH.rowCount} rows deleted.`);
      await client.query('COMMIT'); console.warn(`${logPrefix} !!! ALL DATA DELETED SUCCESSFULLY !!!`);
  ...
  } catch (err) { if (client) await client.query('ROLLBACK'); console.error(`${logPrefix} !!! CRITICAL DB ERROR DURING DELETE ALL: ${err.message}`, err.stack); res.status(500).send('Serverfehler beim Löschen. Transaktion zurückgerollt.'); } finally { if (client) client.release(); }
});
...
app.post('/admin/employees', isAdmin, async (req, res) => {
  ...
  console.log(`${logPrefix} Request received. Data: ${JSON.stringify(req.body)}`);
  // +++ LOGGING ENDE +++
  const { name, mo_hours, di_hours, mi_hours, do_hours, fr_hours } = req.body; const trimmedName = name ? name.trim() : '';
  if (!trimmedName) { console.error(`${logPrefix} ERROR - Empty name.`); return res.status(400).send('Name darf nicht leer sein.'); }
  const hours = [mo_hours, di_hours, mi_hours, do_hours, fr_hours].map(h => parseFloat(h) || 0); if (hours.some(h => h < 0)) { console.error(`${logPrefix} ERROR - Negative hours.`); return res.status(400).send('Stunden dürfen nicht negativ sein.');}
  try {
    ...
    if (err.code === '23505') { console.warn(`${logPrefix} Conflict - Employee name '${trimmedName}' already exists.`); res.status(409).send(`Mitarbeiter '${trimmedName}' existiert bereits.`); }
    else { console.error(`${logPrefix} ERROR - DB Error: ${err.message}`, err.stack); res.status(500).send('Serverfehler Hinzufügen MA.'); }
  }
});
...
app.put('/admin/employees/:id', isAdmin, async (req, res) => {
  ...
  console.log(`${logPrefix} Request received. Data: ${JSON.stringify(req.body)}`);
  // +++ LOGGING ENDE +++
  const { id } = req.params; const { name, mo_hours, di_hours, mi_hours, do_hours, fr_hours } = req.body; const employeeId = parseInt(id); const trimmedName = name ? name.trim() : '';
  if (isNaN(employeeId)) { console.error(`${logPrefix} ERROR - Invalid ID.`); return res.status(400).send('Ungültige ID.'); } if (!trimmedName) { console.error(`${logPrefix} ERROR - Empty name.`); return res.status(400).send('Name darf nicht leer sein.'); }
  const hours = [mo_hours, di_hours, mi_hours, do_hours, fr_hours].map(h => parseFloat(h) || 0); if (hours.some(h => h < 0)) { console.error(`${logPrefix} ERROR - Negative hours.`); return res.status(400).send('Stunden dürfen nicht negativ sein.');}
  let client;
  try {
      client = await db.connect(); await client.query('BEGIN'); console.log(`${logPrefix} Transaction started.`);
      console.log(`${logPrefix} Fetching old name...`); const oldNameResult = await client.query('SELECT name FROM employees WHERE id = $1 FOR UPDATE', [employeeId]); if (oldNameResult.rows.length === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee not found.`); return res.status(404).send(`MA ID ${employeeId} nicht gefunden.`); } const oldName = oldNameResult.rows[0].name; const newName = trimmedName;
      console.log(`${logPrefix} Updating employee table... Old: ${oldName}, New: ${newName}`); const updateEmpQuery = `UPDATE employees SET name = $1, mo_hours = $2, di_hours = $3, mi_hours = $4, do_hours = $5, fr_hours = $6 WHERE id = $7;`; await client.query(updateEmpQuery, [newName, ...hours, employeeId]);
      ...
      await client.query('COMMIT'); console.log(`${logPrefix} Transaction committed successfully.`); res.status(200).send('Mitarbeiterdaten aktualisiert.');
  } catch (err) { if (client) await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR during transaction. Rolled back. Error: ${err.message}`, err.stack); if (err.code === '23505') { res.status(409).send(`Name '${trimmedName}' existiert bereits.`); } else { res.status(500).send('Serverfehler Update MA.'); } } finally { if (client) client.release(); }
});
...
app.delete('/admin/employees/:id', isAdmin, async (req, res) => {
  ...
  // +++ LOGGING ENDE +++
  const { id } = req.params; const employeeId = parseInt(id); if (isNaN(employeeId)) { console.error(`${logPrefix} ERROR - Invalid ID.`); return res.status(400).send('Ungültige ID.');}
  let client;
  try {
    client = await db.connect(); await client.query('BEGIN'); console.log(`${logPrefix} Transaction started.`);
    console.log(`${logPrefix} Fetching employee name...`); const nameResult = await client.query('SELECT name FROM employees WHERE id = $1 FOR UPDATE', [employeeId]); if (nameResult.rows.length === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee not found.`); return res.status(404).send(`MA ID ${employeeId} nicht gefunden.`); } const employeeName = nameResult.rows[0].name;
    console.warn(`${logPrefix} Deleting work_hours for ${employeeName}...`); const workHoursDeleteResult = await client.query('DELETE FROM work_hours WHERE LOWER(name) = LOWER($1)', [employeeName.toLowerCase()]); console.warn(`${logPrefix} -> ${workHoursDeleteResult.rowCount} work_hours rows deleted.`);
    console.warn(`${logPrefix} Deleting employee ${employeeName} (ID: ${employeeId}) from employees table (cascades to absences, monthly_balance)...`); const deleteEmpResult = await client.query('DELETE FROM employees WHERE id = $1', [employeeId]);
    if (deleteEmpResult.rowCount > 0) { await client.query('COMMIT'); console.warn(`${logPrefix} !!! Employee and related data deleted successfully. Transaction committed. !!!`); res.status(200).send('Mitarbeiter und Daten gelöscht.'); }
    else { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee delete failed (rowCount=0). Rolled back.`); res.status(404).send(`MA ID ${employeeId} nicht gelöscht (nicht gefunden?).`); }
  } catch (err) { if (client) await client.query('ROLLBACK'); console.error(`${logPrefix} !!! CRITICAL ERROR during employee delete. Rolled back. Error: ${err.message}`, err.stack); if (err.code === '23503') { res.status(409).send('FK Fehler: Abhängige Daten existieren (sollte nicht passieren mit CASCADE).'); } else { res.status(500).send('Serverfehler Löschen MA.'); } } finally { if (client) client.release(); }
});
// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging) - Teil 6
...
app.get('/calculate-monthly-balance', isAdmin, async (req, res) => {
  ...
  console.log(`${logPrefix} Request received.`);
  if (!name || !year || !month || isNaN(parseInt(year)) || String(parseInt(year)).length !== 4 || isNaN(parseInt(month)) || month < 1 || month > 12) { console.error(`${logPrefix} ERROR - Invalid input.`); return res.status(400).json({ message: "Ungültige Eingabe: Name, Jahr (YYYY) und Monat (1-12) erforderlich." }); }
  try {
    console.log(`${logPrefix} Calling calculateMonthlyData...`);
    const result = await calculateMonthlyData(db, name, year, month); // Hat eigenes Logging
    console.log(`${logPrefix} calculateMonthlyData successful. Sending response.`);
    res.json(result);
  } catch (err) { console.error(`${logPrefix} ERROR - Error during processing: ${err.message}`); if (err.message && err.message.toLowerCase().includes("nicht gefunden")) { res.status(404).json({ message: err.message }); } else { res.status(500).json({ message: `Serverfehler bei der Monatsberechnung: ${err.message}` }); } }
});
...
app.get('/calculate-period-balance', isAdmin, async (req, res) => {
  ...
  console.log(`${logPrefix} Request received.`);
  if (!name || !year || isNaN(parseInt(year)) || String(parseInt(year)).length !== 4) { console.error(`${logPrefix} ERROR - Invalid name or year.`); return res.status(400).json({ message: "Ungültige Eingabe: Name und Jahr (YYYY) erforderlich." }); }
  const periodTypeUpper = periodType ? periodType.toUpperCase() : null;
  if (!periodTypeUpper || !['QUARTER', 'YEAR'].includes(periodTypeUpper)) { console.error(`${logPrefix} ERROR - Invalid periodType.`); return res.status(400).json({ message: "Ungültiger Periodentyp. Erlaubt sind 'QUARTER' oder 'YEAR'." }); }
  let parsedPeriodValue = null;
  if (periodTypeUpper === 'QUARTER') {
      parsedPeriodValue = parseInt(periodValue);
      if (!periodValue || isNaN(parsedPeriodValue) || parsedPeriodValue < 1 || parsedPeriodValue > 4) { console.error(`${logPrefix} ERROR - Invalid periodValue for QUARTER.`); return res.status(400).json({ message: "Ungültiges Quartal (1-4) für Periodentyp 'QUARTER' erforderlich." }); }
  }
  try {
    console.log(`${logPrefix} Calling calculatePeriodData...`);
    const result = await calculatePeriodData(db, name, year, periodTypeUpper, parsedPeriodValue); // Hat eigenes Logging
    console.log(`${logPrefix} calculatePeriodData successful. Sending response.`);
    res.json(result);
  } catch (err) { console.error(`${logPrefix} ERROR - Error during processing: ${err.message}`); if (err.message && err.message.toLowerCase().includes("nicht gefunden")) { res.status(404).json({ message: err.message }); } else { res.status(500).json({ message: `Serverfehler bei der Periodenberechnung: ${err.message}` }); } }
});
...
app.get('/admin/absences', isAdmin, async (req, res) => {
    ...
    console.log(`${logPrefix} Request received.`);
    if (!employeeId || isNaN(empIdInt)) { console.error(`${logPrefix} ERROR - Invalid employeeId.`); return res.status(400).json({ message: 'Gültige numerische employeeId als Query-Parameter erforderlich.' }); }
    try {
        ...
        const formattedResult = result.rows.map(row => ({ ...row, date: (row.date instanceof Date) ? row.date.toISOString().split('T')[0] : String(row.date) }));
        res.json(formattedResult);
    } catch (err) { console.error(`${logPrefix} ERROR - Error during processing: ${err.message}`, err.stack); res.status(500).json({ message: 'Serverfehler beim Laden der Abwesenheiten.' }); }
});
...
app.post('/admin/absences', isAdmin, async (req, res) => {
    ...
    const empIdInt = parseInt(employeeId); if (!employeeId || isNaN(empIdInt)) { console.error(`${logPrefix} ERROR - Invalid employeeId.`); return res.status(400).json({ message: 'Gültige numerische employeeId erforderlich.' }); }
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) { const fd = targetDate.toLocaleDateString('de-DE',{weekday: 'long', timeZone:'UTC'}); console.warn(`${logPrefix} Attempt to book absence on weekend (${fd}).`); return res.status(400).json({ message: `Abwesenheiten können nicht am Wochenende (${fd}) gebucht werden.` }); }
    if (normalizedAbsenceType === 'PUBLIC_HOLIDAY') { const isHoliday = hd.isHoliday(targetDate); if (!isHoliday) { const fd = targetDate.toLocaleDateString('de-DE',{timeZone:'UTC'}); console.warn(`${logPrefix} Attempt to book non-holiday ${fd} as PUBLIC_HOLIDAY for MA ${empIdInt}.`); return res.status(400).json({ message: `Das Datum ${fd} ist laut System kein gesetzlicher Feiertag in NRW.` }); } }
    let client;
    try {
        client = await db.connect(); await client.query('BEGIN'); console.log(`${logPrefix} Transaction started.`);
        console.log(`${logPrefix} Fetching employee data for ID ${empIdInt}...`); const empResult = await client.query('SELECT * FROM employees WHERE id = $1', [empIdInt]); if (empResult.rows.length === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee not found.`); return res.status(404).json({ message: `Mitarbeiter mit ID ${empIdInt} nicht gefunden.` }); } const employeeData = empResult.rows[0];
        console.log(`${logPrefix} Calculating expected hours for ${date}...`); const expectedHoursForDay = getExpectedHours(employeeData, date); let credited_hours = expectedHoursForDay;
        if (normalizedAbsenceType !== 'PUBLIC_HOLIDAY' && expectedHoursForDay <= 0) { const fd = targetDate.toLocaleDateString('de-DE',{weekday: 'long', timeZone:'UTC'}); await client.query('ROLLBACK'); console.warn(`${logPrefix} Cannot book absence, employee has 0 expected hours on ${fd}.`); return res.status(400).json({ message: `Buchung nicht möglich: Mitarbeiter ${employeeData.name} hat an diesem Tag (${fd}) keine Soll-Stunden.` }); }
        credited_hours = Math.max(0, credited_hours);
        console.log(`${logPrefix} Inserting absence (Type: ${normalizedAbsenceType}, Credited: ${credited_hours})...`); const insertQuery = `INSERT INTO absences (employee_id, date, absence_type, credited_hours, comment) VALUES ($1, $2, $3, $4, $5) RETURNING id, date, absence_type, credited_hours, comment;`; const insertResult = await client.query(insertQuery, [empIdInt, date, normalizedAbsenceType, credited_hours, comment || null]);
        await client.query('COMMIT'); console.log(`${logPrefix} Transaction committed. Absence ID: ${insertResult.rows[0].id}`);
    ...
    } catch (err) { if (client) await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR during transaction. Rolled back. Error: ${err.message}`, err.stack); if (err.code === '23505') { const fd = new Date(date+'T00:00:00Z').toLocaleDateString('de-DE',{timeZone:'UTC'}); res.status(409).json({ message: `Für diesen Mitarbeiter existiert bereits ein Abwesenheitseintrag am ${fd}.` }); } else if (err.code === '23503') { res.status(404).json({ message: `Mitarbeiter mit ID ${empIdInt} nicht gefunden (FK Fehler).`}); } else { res.status(500).json({ message: 'Serverfehler beim Hinzufügen der Abwesenheit.' }); } } finally { if (client) client.release(); }
});
...
app.delete('/admin/absences/:id', isAdmin, async (req, res) => {
    ...
    console.log(`${logPrefix} Request received.`);
    // +++ LOGGING ENDE +++
    const { id } = req.params; const absenceId = parseInt(id); if (isNaN(absenceId)) { console.error(`${logPrefix} ERROR - Invalid ID.`); return res.status(400).send('Ungültige Abwesenheits-ID.');}
    try {
        console.log(`${logPrefix} Deleting absence from DB...`);
        const result = await db.query('DELETE FROM absences WHERE id = $1', [absenceId]);
        if (result.rowCount > 0) { console.log(`${logPrefix} Delete successful.`); res.status(200).send('Abwesenheitseintrag erfolgreich gelöscht.'); }
        else { console.warn(`${logPrefix} Absence not found.`); res.status(404).send(`Abwesenheitseintrag mit ID ${absenceId} nicht gefunden.`); }
    } catch (err) { console.error(`${logPrefix} ERROR - DB Error: ${err.message}`, err.stack); res.status(500).send('Serverfehler beim Löschen der Abwesenheit.'); }
});
...
app.post('/admin/generate-holidays', isAdmin, async (req, res) => {
    ...
    console.log(`${logPrefix} Request received.`);
    // +++ LOGGING ENDE +++
    const { year } = req.body; const currentYear = new Date().getFullYear(); const minYear = currentYear - 5; const maxYear = currentYear + 5; const targetYear = parseInt(year);
    if (!year || isNaN(targetYear) || targetYear < minYear || targetYear > maxYear) { console.error(`${logPrefix} ERROR - Invalid year.`); return res.status(400).json({ message: `Ungültiges oder fehlendes Jahr. Bitte ein Jahr zwischen ${minYear} und ${maxYear} angeben.` }); }
    console.log(`${logPrefix} Starting holiday generation for NRW, year ${targetYear}...`);
    let client; let generatedCount = 0; let skippedCount = 0; let processedEmployees = 0;
    try {
        client = await db.connect(); await client.query('BEGIN'); console.log(`${logPrefix} Transaction started.`);
        console.log(`${logPrefix} Fetching employees...`); const empResult = await client.query('SELECT id, name, mo_hours, di_hours, mi_hours, do_hours, fr_hours FROM employees ORDER BY name'); const employees = empResult.rows; processedEmployees = employees.length; if (processedEmployees === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} No employees found. Aborting generation.`); return res.status(404).json({ message: "Keine Mitarbeiter gefunden, für die Feiertage generiert werden könnten." }); } console.log(`${logPrefix} -> ${processedEmployees} employees found.`);
        console.log(`${logPrefix} Fetching public holidays for ${targetYear}...`); const holidaysOfYear = hd.getHolidays(targetYear); const publicHolidays = holidaysOfYear.filter(h => h.type === 'public'); console.log(`${logPrefix} -> ${publicHolidays.length} public holidays found.`);
        const insertQuery = `INSERT INTO absences (employee_id, date, absence_type, credited_hours, comment) VALUES ($1, $2, 'PUBLIC_HOLIDAY', $3, $4) ON CONFLICT (employee_id, date) DO NOTHING;`;
        for (const holiday of publicHolidays) {
            const holidayDateString = holiday.date.split(' ')[0]; const holidayDate = new Date(holidayDateString + 'T00:00:00Z'); const dayOfWeek = holidayDate.getUTCDay();
            if (dayOfWeek === 0 || dayOfWeek === 6) { continue; } // Skip weekends
            for (const employee of employees) {
                const expectedHours = getExpectedHours(employee, holidayDateString);
                if (expectedHours > 0) {
                    const result = await client.query(insertQuery, [ employee.id, holidayDateString, expectedHours, holiday.name ]);
                    if (result.rowCount > 0) { generatedCount++; } else { skippedCount++; }
                }
            }
        }
        await client.query('COMMIT'); console.log(`${logPrefix} Transaction committed. Generated: ${generatedCount}, Skipped: ${skippedCount}.`);
        res.status(200).json({ message: `Feiertage für ${targetYear} erfolgreich generiert/geprüft.`, generated: generatedCount, skipped: skippedCount, employees: processedEmployees });
    } catch (err) { if (client) await client.query('ROLLBACK'); console.error(`${logPrefix} !!! CRITICAL ERROR during holiday generation. Rolled back. Error: ${err.message}`, err.stack); res.status(500).json({ message: `Serverfehler bei der Feiertagsgenerierung: ${err.message}` }); } finally { if (client) client.release(); }
});
...
app.listen(port, () => {
  ...
  console.log(` Session Cookie Settings: secure=${process.env.NODE_ENV === 'production'}, sameSite='${session.options?.cookie?.sameSite || 'lax (default?)'}'`); // Versuche Cookie Settings zu loggen
  console.log(`=======================================================`);
});
