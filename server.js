// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging + Aufsteigende Sortierung + Verhinderung Doppelbuchung + Wochentag im CSV + Global Error Handler + Mitarbeiter-Passwörter)
...
// Stelle sicher, dass diese Dateien existieren und korrekt exportieren
...
// Angepasste convertToCSV Funktion (bleibt inhaltlich gleich)
...
app.post('/log-start', async (req, res) => {
    ...
    try {
        ...
        // Prüfen, ob bereits ein abgeschlossener Eintrag für diesen Tag existiert (Verhinderung Mehrfachbuchung)
        ...
        if (checkCompleteResult.rows.length > 0) {
            console.warn(`BLOCKIERT: Mitarbeiter ${dbEmployeeName} versucht erneuten Start am ${date}, obwohl bereits abgeschlossene Buchung existiert.`);
...
app.get('/summary-hours', async (req, res) => {
    ...
    try {
        ...
        // Monatsstunden (bis zum abgefragten Tag)
        ...
        const lastDayForQuery = date; // Bis zum angegebenen Datum im Monat
...
app.post("/admin-login", (req, res) => {
    ...
    if (password === adminPassword) { // Einfacher String-Vergleich - Für echte Anwendungen unsicher!
...
app.post("/admin-logout", isAdmin, (req, res) => {
    if (req.session) {
        ...
        req.session.destroy(err => {
            ...
            res.clearCookie('connect.sid'); // Cookie im Browser löschen
    ...
    try {
        ...
        if (year && month) {
            ...
            try {
                ...
                const endDate = new Date(Date.UTC(parsedYear, parsedMonth, 1)); // Nächster Monat, 1. Tag
                ...
                whereClauses.push(`w.date < $${paramIndex++}`); // Exklusiv Enddatum
        ...
        finalQuery += ` ORDER BY w.date ASC, e.name ASC, w.starttime ASC NULLS FIRST;`; // Aufsteigende Sortierung
        ...
        const formattedRows = result.rows.map(row => ({
            ...row,
            date: row.date instanceof Date ? row.date.toISOString().split('T')[0] : row.date // Datum formatieren
        }));
        res.json(formattedRows);

    } catch (err) {
        console.error(`${logPrefix} ERROR - DB or processing error: ${err.message}`);
        next(err); // Fehler an globalen Handler weitergeben
    }
});
...
app.get('/admin-download-csv', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (employeeId && employeeId !== 'all' && employeeId !== '') {
             ...
             } catch {} // Fehler ignorieren, wenn Name nicht geholt werden kann
        } else {
            filterDesc = "alle_MA";
        }
        ...
        const csvData = await convertToCSV(db, result.rows); // DB übergeben
        ...
        console.error(`${logPrefix} ERROR - DB or CSV generation error: ${err.message}`);
        next(err);
    }
});
...
app.put('/api/admin/update-hours', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (result.rowCount > 0) {
            ...
            res.status(404).send(`Eintrag ID ${entryId} nicht aktualisiert.`);
        }
    } catch (err) {
        console.error(`${logPrefix} ERROR - DB or processing error: ${err.message}`);
        next(err);
...
// --- Mitarbeiterverwaltung (ANGEPASST für Passwörter) ---
app.get('/admin/employees', isAdmin, async (req, res, next) => {
    try {
        ...
        // Passwort-Hash wird hier NICHT abgefragt oder gesendet!
        ...
        console.error(`[ROUTE:/admin/employees GET] ERROR - DB Error: ${err.message}`, err.stack);
        next(err);
    }
});

// ANGEPASST für Passwort
app.post('/admin/employees', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (!trimmedName) {
            console.error(`${logPrefix} ERROR - Empty name.`);
            return res.status(400).send('Name darf nicht leer sein.');
        }
        if (!password || password.length < 6) {
             console.error(`${logPrefix} ERROR - Password missing or too short.`);
             return res.status(400).send('Passwort fehlt oder ist zu kurz (mind. 6 Zeichen).');
        }

        const hours = [mo_hours, di_hours, mi_hours, do_hours, fr_hours].map(h => parseFloat(h) || 0);
        if (hours.some(h => h < 0)) {
            console.error(`${logPrefix} ERROR - Negative hours.`);
            return res.status(400).send('Stunden dürfen nicht negativ sein.');
        }

        const saltRounds = 10;
        const passwordHash = await bcrypt.hash(password, saltRounds);
        console.log(`${logPrefix} Password hashed successfully.`);

        console.log(`${logPrefix} Inserting new employee '${trimmedName}'...`);
        const query = `INSERT INTO employees (name, password_hash, mo_hours, di_hours, mi_hours, do_hours, fr_hours) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id, name, mo_hours, di_hours, mi_hours, do_hours, fr_hours;`;
        const result = await db.query(query, [trimmedName, passwordHash, ...hours]);

        console.log(`${logPrefix} Insert successful. ID: ${result.rows[0].id}`);
        res.status(201).json(result.rows[0]); // Passwort-Hash nicht zurückgeben
...
app.put('/admin/employees/:id', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (!trimmedName) { await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR - Empty name.`); return res.status(400).send('Name darf nicht leer sein.'); }

        if (newPassword && newPassword.length < 6) {
             console.error(`${logPrefix} ERROR - New password too short.`);
             await client.query('ROLLBACK');
             return res.status(400).send('Neues Passwort ist zu kurz (mind. 6 Zeichen).');
        }

        const hours = [mo_hours, di_hours, mi_hours, do_hours, fr_hours].map(h => parseFloat(h) || 0);
        if (hours.some(h => h < 0)) { await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR - Negative hours.`); return res.status(400).send('Stunden dürfen nicht negativ sein.');}

        const oldNameResult = await client.query('SELECT name FROM employees WHERE id = $1 FOR UPDATE', [employeeId]);
        if (oldNameResult.rows.length === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee not found.`); return res.status(404).send(`MA ID ${employeeId} nicht gefunden.`); }
        ...
        let updateQuery = `UPDATE employees SET name = $1, mo_hours = $2, di_hours = $3, mi_hours = $4, do_hours = $5, fr_hours = $6`;
        const queryParams = [newName, ...hours];
        let paramIndex = 7;

        if (newPassword) {
            const saltRounds = 10;
            const passwordHash = await bcrypt.hash(newPassword, saltRounds);
            console.log(`${logPrefix} New password provided, hashing and adding to update.`);
...
// Mitarbeiter löschen (bleibt funktional gleich, CASCADE löscht abhängige Daten)
app.delete('/admin/employees/:id', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        // Namen holen für Logging und ggf. work_hours Löschung (obwohl CASCADE das erledigen sollte)
        ...
        if (nameResult.rows.length === 0) {
            ...
            return res.status(404).send(`MA ID ${employeeId} nicht gefunden.`);
        }
        const employeeName = nameResult.rows[0].name;

        // // Manuelles Löschen von work_hours (optional, da CASCADE vorhanden sein sollte)
        ...
        // Mitarbeiter löschen (CASCADE sollte absences und monthly_balance löschen)
        ...
        if (deleteEmpResult.rowCount > 0) {
            ...
            res.status(200).send('Mitarbeiter und zugehörige Daten gelöscht.');
        } else {
            await client.query('ROLLBACK');
            console.warn(`${logPrefix} Employee delete failed (rowCount=0). Rolled back.`);
            res.status(404).send(`MA ID ${employeeId} nicht gelöscht (nicht gefunden?).`);
        }
    } catch (err) {
        if (client) await client.query('ROLLBACK');
        console.error(`${logPrefix} !!! CRITICAL ERROR during employee delete. Rolled back. Error: ${err.message}`, err.stack);
        // Foreign Key Violation Error (sollte wegen ON DELETE CASCADE nicht passieren)
        if (err.code === '23503') {
...
app.get('/calculate-monthly-balance', isAdmin, async (req, res, next) => {
    try {
        ...
        const logPrefix = `[ROUTE:/calculate-monthly-balance] MA: ${req.query.name}, Date: ${req.query.month}/${req.query.year}, Session: ${req.sessionID} -`;
...
app.get('/calculate-period-balance', isAdmin, async (req, res, next) => {
    try {
        ...
        const logPrefix = `[ROUTE:/calculate-period-balance] MA: ${req.query.name}, Year: ${req.query.year}, Type: ${req.query.periodType}, Val: ${req.query.periodValue}, Session: ${req.sessionID} -`;
        console.error(`${logPrefix} ERROR - Error during processing: ${err.message}`);
        if (err.message && err.message.toLowerCase().includes("nicht gefunden")) {
             res.status(404).json({ message: err.message });
        } else {
...
// --- Abwesenheiten (unverändert) ---
// GET /admin/absences
app.get('/admin/absences', isAdmin, async (req, res, next) => {
    try {
        const { employeeId } = req.query;
        const empIdInt = parseInt(employeeId);
        const logPrefix = `[ROUTE:/admin/absences GET] EmpID: ${employeeId}, Session: ${req.sessionID} -`;
        console.log(`${logPrefix} Request received.`);
        ...
        const logPrefix = `[ROUTE:/admin/absences GET] EmpID: ${req.query.employeeId}, Session: ${req.sessionID} -`;
...
app.post('/admin/absences', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        const empIdInt = parseInt(employeeId);
        if (!employeeId || isNaN(empIdInt)) { await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR - Invalid employeeId.`); return res.status(400).json({ message: 'Gültige numerische employeeId erforderlich.' }); }
        if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) { await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR - Invalid date.`); return res.status(400).json({ message: 'Gültiges Datum im Format YYYY-MM-DD erforderlich.' }); }
        if (!absenceType || !['VACATION', 'SICK', 'PUBLIC_HOLIDAY'].includes(absenceType.toUpperCase())) { await client.query('ROLLBACK'); console.error(`${logPrefix} ERROR - Invalid absenceType.`); return res.status(400).json({ message: "Ungültiger absenceType. Erlaubt: 'VACATION', 'SICK', 'PUBLIC_HOLIDAY'." }); }

        const normalizedAbsenceType = absenceType.toUpperCase();
        const targetDate = new Date(date + 'T00:00:00Z');
        const dayOfWeek = targetDate.getUTCDay(); // 0 = Sonntag, 6 = Samstag

        // Keine Buchung am Wochenende (außer Feiertag)
        ...
        // Prüfung, ob es wirklich ein Feiertag ist, wenn Typ 'PUBLIC_HOLIDAY'
        if (normalizedAbsenceType === 'PUBLIC_HOLIDAY') {
            const isHoliday = hd.isHoliday(targetDate);
            if (!isHoliday || isHoliday.type !== 'public') {
                 const fd = targetDate.toLocaleDateString('de-DE',{timeZone:'UTC'});
                 console.warn(`${logPrefix} Attempt to book non-public-holiday ${fd} as PUBLIC_HOLIDAY for MA ${empIdInt}. Actual type: ${isHoliday?.type || 'none'}`);
                 await client.query('ROLLBACK');
        ...
        if (empResult.rows.length === 0) { await client.query('ROLLBACK'); console.warn(`${logPrefix} Employee not found.`); return res.status(404).json({ message: `Mitarbeiter mit ID ${empIdInt} nicht gefunden.` }); }
        const employeeData = empResult.rows[0];

        console.log(`${logPrefix} Calculating expected hours for ${date}...`);
        const expectedHoursForDay = getExpectedHours(employeeData, date);
        let credited_hours = expectedHoursForDay;

        // Buchung nicht möglich, wenn MA an dem Tag 0 Soll-Stunden hat (außer bei Feiertag?)
        // Überlegung: Sollte man Feiertag auch buchen dürfen, wenn 0 Soll? Aktuell: Ja.
        ...
        credited_hours = Math.max(0, credited_hours); // Sicherstellen, dass nicht negativ
        ...
        const createdAbsence = {
            ...insertResult.rows[0],
            date: insertResult.rows[0].date.toISOString().split('T')[0],
...
app.delete('/admin/absences/:id', isAdmin, async (req, res, next) => {
    try {
        ...
        if (isNaN(absenceId)) { console.error(`${logPrefix} ERROR - Invalid ID.`); return res.status(400).send('Ungültige Abwesenheits-ID.');}
...
app.post('/admin/generate-holidays', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        for (const holiday of publicHolidays) {
            const holidayDateString = holiday.date.split(' ')[0]; // Nur YYYY-MM-DD
            const holidayDate = new Date(holidayDateString + 'T00:00:00Z');
            const dayOfWeek = holidayDate.getUTCDay(); // 0 = Sonntag, 6 = Samstag

            // Nur eintragen, wenn es ein Wochentag ist (Mo-Fr)
            if (dayOfWeek !== 0 && dayOfWeek !== 6) {
                 for (const employee of employees) {
                    const expectedHours = getExpectedHours(employee, holidayDateString);
                    // Nur eintragen, wenn der Mitarbeiter an dem Tag Soll-Stunden hat
                    if (expectedHours > 0) {
                        ...
                        if (result.rowCount > 0) {
                            ...
                            skippedCount++; // Bereits vorhanden oder anderer Konflikt
                        }
                    }
                 }
            }
...
// DELETE /admin/delete-public-holidays
app.delete('/admin/delete-public-holidays', isAdmin, async (req, res, next) => {
    try {
        ...
        res.status(200).json({ message: `Erfolgreich ${result.rowCount} Abwesenheitseinträge vom Typ 'Feiertag' gelöscht.` });
    } catch (err) {
        const logPrefix = `[ROUTE:/admin/delete-public-holidays DELETE] Session: ${req.sessionID} -`;
...
// --- PDF Router ---
try {
    // Stelle sicher, dass die Funktion eine Router-Instanz zurückgibt
...
app.use((err, req, res, next) => {
    ...
    if (!res.headersSent) {
         // Sende generische Fehlermeldung, vermeide Details in Produktion
         res.status(500).send('Ein unerwarteter interner Serverfehler ist aufgetreten.');
    } else {
         next(err); // An Standard-Express-Handler übergeben
    }
});
...
app.listen(port, () => {
    ...
    if(db && typeof db.options === 'object') {
        // Versuche DB-Infos aus Umgebungsvariablen oder Pool-Optionen zu holen
        const host = process.env.PGHOST || db.options.host || '??';
        const port = process.env.PGPORT || db.options.port || '??';
        const database = process.env.PGDATABASE || db.options.database || '??';
        console.log(` Datenbank verbunden (Pool erstellt): Host=${host}, Port=${port}, DB=${database}`);
    } else if (db) {
    ...
    // Sicherer Zugriff auf Session-Cookie-Einstellungen
    let sessionCookieSecure = process.env.NODE_ENV === 'production';
    let sessionCookieSameSite = 'lax'; // Standardwert
    try {
        // Greife nur auf session.cookie zu, wenn session existiert und cookie ein Objekt ist
    ...
    } catch (e) { console.warn("Warnung: Konnte Session-Cookie-Details nicht vollständig lesen.", e.message); }
