...
// --> NEU: Proxy Vertrauen schenken (wichtig für Railway/Heroku etc.)
// Diese Zeile sollte relativ weit oben stehen, nach `const app = express();`
...
app.use(session({
  ...
  resave: false,
  // --> GEÄNDERT: saveUninitialized auf false (empfohlen)
  saveUninitialized: false,
  cookie: {
    secure: process.env.NODE_ENV === 'production', // Korrekt für HTTPS auf Railway
    // --> NEU: sameSite explizit setzen
    sameSite: 'lax', // 'lax' ist ein guter Standardwert
    httpOnly: true, // Standardmäßig true, gut für Sicherheit
    // maxAge: 1000 * 60 * 60 * 24 // Optional: Session-Lebensdauer festlegen (z.B. 1 Tag)
  }
}));
...
const db = new Pool({
  ...
  ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false // SSL nur in Produktion (z.B. Heroku/Railway)
});
...
function isAdmin(req, res, next) {
  // Sicherstellen, dass session existiert und isAdmin true ist
  if (req.session && req.session.isAdmin === true) {
    next();
  } else {
    // Detaillierteres Logging für Fehlersuche
...
function convertToCSV(data) {
  ...
  for (const row of data) {
    ...
    // Wichtig: Die 'row' muss hier die employee Stunden (mo_hours etc.) enthalten, was der JOIN in /admin-download-csv sicherstellt
...
app.delete('/delete-hours', isAdmin, async (req, res) => {
  try { await db.query('DELETE FROM work_hours'); res.send('Alle Arbeitszeiten erfolgreich gelöscht.'); }
  catch (err) { console.error("DB Fehler in /delete-hours:", err); res.status(500).send('Fehler beim Löschen der Daten.'); }
});

// --- Admin Login --- (Mit explizitem Session-Save)
app.post('/admin-login', (req, res) => {
  const { password } = req.body;
  if (password && password === "admin") { // Passwortprüfung hardcoded "admin"
    req.session.isAdmin = true;
    req.session.save(err => { // Explizites Speichern
      if (err) {
      ...
      console.log("Session gespeichert, isAdmin:", req.session.isAdmin);
...
app.get('/calculate-monthly-balance', isAdmin, async (req, res) => {
  ...
  if (!name || !year || !month || isNaN(parseInt(year)) || isNaN(parseInt(month))) { return res.status(400).send("Bitte gültigen Namen, Jahr und Monat angeben."); }
  ...
  try {
    ...
    const workResult = await db.query( `SELECT date, hours FROM work_hours WHERE LOWER(name) = LOWER($1) AND date >= $2 AND date < $3`, [name, startDateStr, endDateStr] );
    const workEntries = workResult.rows;

    let totalDifference = 0;
    workEntries.forEach(entry => { const expected = getExpectedHours(employee, entry.date); totalDifference += (entry.hours || 0) - expected; });
...
app.get('/admin-work-hours', isAdmin, (req, res) => {
  const query = `SELECT id, name, date, hours, (break_time * 60) AS break_time, comment, TO_CHAR(starttime, 'HH24:MI') AS "startTime", TO_CHAR(endtime, 'HH24:MI') AS "endTime" FROM work_hours ORDER BY date DESC, name ASC, starttime ASC;`;
  db.query(query)
...
app.get('/admin-download-csv', isAdmin, async (req, res) => {
  const query = `SELECT w.*, e.mo_hours, e.di_hours, e.mi_hours, e.do_hours, e.fr_hours FROM work_hours w LEFT JOIN employees e ON LOWER(w.name) = LOWER(e.name) ORDER BY w.date ASC, w.name ASC, w.starttime ASC;`;
  try {
