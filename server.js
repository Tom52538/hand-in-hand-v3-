// server.js - KORRIGIERTE VERSION (Trust Proxy + Diagnose-Logging + Aufsteigende Sortierung + Verhinderung Doppelbuchung + Wochentag im CSV + Global Error Handler + isAdmin für PDF Route)
...
// Stelle sicher, dass der Pfad zu calculationUtils korrekt ist
        ...
        if(employeeData && dateForCalc && typeof getExpectedHours === 'function') {
            try {
                ...
                if (absenceCheck.rows.length === 0) { // Nur wenn KEINE Abwesenheit
                    expectedHours = getExpectedHours(employeeData, dateForCalc); // Hier wird String übergeben, getExpectedHours sollte das können
                }
...
// Middleware zur Prüfung ob Admin eingeloggt ist (global hier definiert)
  ...
  try {
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS work_hours (
        id SERIAL PRIMARY KEY,
        name TEXT NOT NULL, -- Sollte auf employees(name) verweisen, aber wegen Namensänderung schwierig mit FK
        date DATE NOT NULL,
        starttime TIME,
        endtime TIME,
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS monthly_balance (
        ...
        year_month DATE NOT NULL, -- Speichert den Ersten des Monats
...
app.put('/api/admin/update-hours', isAdmin, async (req, res, next) => {
    ...
    try {
        ...
        if (result.rowCount > 0) {
            ...
            res.status(404).send(`Eintrag ID ${entryId} nicht aktualisiert.`); // Sollte nicht passieren
        }
    } catch (err) {
        console.error(`${logPrefix} ERROR - DB or processing error: ${err.message}`);
        next(err);
...
// Fängt alle Fehler ab, die mit next(err) übergeben werden.
app.use((err, req, res, next) => {
    ...
    // Sende generische Fehlermeldung, es sei denn, ein Statuscode wurde bereits gesetzt
    if (!res.headersSent) {
         const statusCode = res.statusCode && res.statusCode >= 400 ? res.statusCode : 500; // Nutze gesetzten Status oder 500
         // Sende die Fehlermeldung aus dem Error-Objekt nur, wenn es kein 500er ist (oder im Development)
         const message = (statusCode === 500 && process.env.NODE_ENV === 'production') ? 'Ein interner Serverfehler ist aufgetreten.' : (err.message || 'Ein unerwarteter Fehler ist aufgetreten.');
...
app.listen(port, () => {
    ...
    console.log(` Session Cookie Settings: secure=${process.env.NODE_ENV === 'production'}, sameSite='${app.get('session')?.cookie?.sameSite || 'lax (Standard?)'}'`);
    console.log(`=======================================================`);
});
